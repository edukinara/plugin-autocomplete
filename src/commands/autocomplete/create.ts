...
export default class Create extends AutocompleteBase {
  ...
  private async createFiles() {
    ...
    await fs.writeFile(
      this.bashCompletionFunctionPath,
      this.bashCompletionFunction
    );
    await fs.writeFile(this.zshSetupScriptPath, this.zshSetupScript);
    await fs.writeFile(
      this.zshCompletionFunctionPath,
      this.zshCompletionFunction
    );
    if (this.config.shell === "fish") {
      ...
      await fs.writeFile(
        this.fishCompletionFunctionPath,
        this.fishCompletionFunction
      );
    }
  }
  ...
  private genZshFlagSpecs(Klass: any): string {
    return Object.keys(Klass.flags || {})
      ...
      .map((flag) => {
        ...
        const completion = `--${name}[${sanitizeDescription(
          f.description
        )}]${valueCmpl}`;
        return `"${completion}"`;
      })
...
function  __fish_${cliBin}_using_command
  ...
  return 1
    ...
    for (const command of this.commands) {
      completions.push(
        `complete -f -c ${cliBin} -n '__fish_${cliBin}_needs_command' -a ${command.id} -d "${command.description}"`
      );
      const flags = command.flags || {};
      Object.keys(flags)
